...
std::vector<std::string> resolve_manifest(const std::string& manifest_path,
  ...
  for (size_t i = 0; i < arch_count; ++i) {
    ...
    std::string requested_arch = arch_list[i];
    std::string base_arch = strip_target_prefix(arch_list[i]);

    KPACK_DEBUG(cache, "looking for arch: %s (base: %s)",
                requested_arch.c_str(), base_arch.c_str());
...
kpack_error_t kpack_load_code_object(kpack_cache_t cache,
  ...
  // For multi-TU binaries, the caller passes an indexed path (e.g.,
  // "/path/lib.so#1"). Extract the index and append it to the embedded
  // kernel_name for TOC lookup. The embedded kernel_name has the relative path
  // that matches TOC keys.
  ...
  for (size_t i = 0; i < arch_count; ++i) {
    ...
    // Use lookup_key (from binary_path) which includes bundle index for
    // multi-TU Use base_arch (stripped) since that's how kernels are stored in
    // the archive
